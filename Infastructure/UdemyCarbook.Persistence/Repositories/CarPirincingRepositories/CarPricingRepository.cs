...
namespace UdemyCarbook.Persistence.Repositories.CarPirincingRepositories
{
    public class CarPricingRepository : ICarPricingRepository
    {
        ...
        public async Task<decimal?> GetAmountAsync(int carId, string pricingName)
        {
            return await _context.CarPricings
                .Include(x => x.Piricing)
                .Where(x => x.CarId == carId && x.Piricing.Name == pricingName)
                .Select(x => (decimal?)x.Ammount)
                .FirstOrDefaultAsync();
        }
    }
}
